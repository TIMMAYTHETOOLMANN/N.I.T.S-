...
export class IntegratedNITSCore {
  ...
  /**
   * Analyze document using AI Investigator (GPT-4)
   * Generates AI-powered legal violation analysis with statute citations
   * 
   * @param filePath - Path to document to analyze
   */
  async analyzeUsingAI(filePath: string): Promise<void> {
    ...
    console.log('');
    console.log('═══════════════════════════════════════════════════════════');
    console.log('🤖 AI INVESTIGATOR ANALYSIS');
    console.log('═══════════════════════════════════════════════════════════');
...
Identify any SEC, DOJ, IRS, or financial violations and cite relevant statutes. Provide:
...
4. Recommended enforcement actions`;
      ...
      console.log('🔍 Sending document to AI Investigator (GPT-4)...');
      
      // Get AI analysis
      const aiReport = await analyzeWithAI(prompt);
      ...
      const outputFilename = `ai_${path.basename(filePath, path.extname(filePath))}.md`;
      const outputPath = path.join(outputDir, outputFilename);
      
      // Create formatted report
      const formattedReport = `# 🤖 AI INVESTIGATOR REPORT
...
`;
      ...
      console.log('');
      console.log('✅ AI Investigator analysis complete');
      ...
      const err = error as Error;
      console.error('');
      console.error('❌ AI Investigator analysis failed');
